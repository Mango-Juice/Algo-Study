...
void dfs(int count, pair<int, int> red, pair<int, int> blue, int arr[][MAX], vector<int> ans) {
	if (count == 10) return;
	...
	for (int i = 0; i < 4; i++) {
		...
		if (fail || success) {
			if (success && !fail && count + 1 < answer) {
				answer_val = val;
				answer = count + 1;
			}
			success = false;
		...
		if (!equal(&arr[0][0], &arr[0][0] + (MAX * MAX), &new_arr[0][0])) dfs(count + 1, new_red, new_blue, new_arr, val);
	}
}
...
int main(void) {
	...
	dfs(0, red, blue, arr, ans);
	printf("%d\n", answer < 100 ? answer : -1);

	for (i = 0; i < answer_val.size(); i++) {
		switch (answer_val[i]) {
